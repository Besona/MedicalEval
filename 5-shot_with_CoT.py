...
def get_few_shot_examples(task_name):
    """
    获取指定任务的 5-shot with CoT 示例。
    """
    examples_file = Path('5-shot_instance/5-shot_with_cot_examples.json')
...
def main():
    """
    主函数，负责 5-shot with CoT 实验的流程。
    """
    # 加载模型
    tokenizer, model = load_baichuan_model()
    model_name = "Baichuan-13B"
    evaluation_type = '5-shot with CoT'
    ...
    # 输入初始 system prompt 和 CoT system prompt
    system_prompt = input("请输入初始 system prompt：").strip()
    cot_system_prompt = input("请输入 CoT system prompt：").strip()
    ...
    for task_name in tasks:
        ...
        # 获取 5-shot with CoT 示例
        examples = get_few_shot_examples(task_name)
        if not examples:
            print(f"任务 {task_name} 没有找到 5-shot with CoT 示例，跳过该任务。")
            continue
        ...
        for i, patient in enumerate(task_data[:3]):
            ...
            # 第一次模型推理
            messages_first = [{"role": "system", "content": system_prompt}]
            messages_first.append({"role": "user", "content": few_shot_prompt})
            input_text_first = " ".join([message['content'] for message in messages_first])
            print("第一次输入的问题: ", input_text_first)
            response_first = model.chat(tokenizer, [{"role": "user", "content": input_text_first}])
            print("第一次模型回答：", response_first)

            # 第二次模型推理，使用 CoT system prompt 和第一次的回答
            cot_question = response_first  # 使用第一次的回答作为新的输入
            messages_second = [{"role": "system", "content": cot_system_prompt}]
            messages_second.append({"role": "user", "content": cot_question})
            input_text_second = " ".join([message['content'] for message in messages_second])
            print("第二次输入的问题: ", input_text_second)
            response_second = model.chat(tokenizer, [{"role": "user", "content": input_text_second}])
            print("第二次模型回答：", response_second)

            # 存储结果
            patient['response_first'] = response_first
            patient['response_second'] = response_second
            # 提取答案
            final_extract = extract_option(response_second, patient['options'])
            patient['extract'] = final_extract
            print('最终提取的答案：' + final_extract + '\n' + '-' * 100)
